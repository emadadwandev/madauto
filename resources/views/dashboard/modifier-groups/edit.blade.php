        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('dashboard.modifier-groups.update', ['modifierGroup' => $modifierGroup, 'subdomain' => request()->route('subdomain')]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description"
                                      id="description"
                                      rows="2"
                                      class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $modifierGroup->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <select name="selection_type"
                                    ...
                                    <option value="single" {{ old('selection_type', $modifierGroup->selection_type) === 'single' ? 'selected' : '' }}>Single (Radio buttons)</option>
                                    <option value="multiple" {{ old('selection_type', $modifierGroup->selection_type) === 'multiple' ? 'selected' : '' }}>Multiple (Checkboxes)</option>
                                </select>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <input type="number"
                                       name="min_selections"
                                       id="min_selections"
                                       value="{{ old('min_selections', $modifierGroup->min_selections) }}"
                                       min="0"
                                       required
                        ...
                        <div class="mb-6 space-y-3">
                            <div class="flex items-center">
                                <input type="checkbox"
                                       ...
                                       id="is_required"
                                       value="1"
                                       {{ old('is_required', $modifierGroup->is_required) ? 'checked' : '' }}
                        ...
                        <div class="mb-6 border-t border-gray-200 pt-6">
                            ...
                            @php
                                $selectedModifiers = old('modifiers', $modifierGroup->modifiers->pluck('id')->toArray());
                            @endphp
                            ...
                            @if($modifiers->count() > 0)
                                    ...
                                    @foreach($modifiers as $modifier)
                                        <label class="flex items-center p-3 bg-gray-50 hover:bg-gray-100 rounded cursor-pointer">
                                            <input type="checkbox"
                                                   name="modifiers[]"
                                                   value="{{ $modifier->id }}"
                                                   {{ in_array($modifier->id, $selectedModifiers) ? 'checked' : '' }}
                                                   class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                <p class="mt-2 text-sm text-gray-500">Select which modifiers belong to this group. Currently selected: {{ count($selectedModifiers) }}</p>
                            @else
