    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="p-6">
                    <form method="POST" action="{{ route('sync-logs.retry-all', ['subdomain' => request()->route('subdomain')]) }}" class="inline" onsubmit="return confirm('Retry all failed orders?')">
                        @csrf
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="p-6">
                    <form method="GET" action="{{ route('sync-logs.index', ['subdomain' => request()->route('subdomain')]) }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div>
                        ...
                        <div class="md:col-span-5 flex gap-2">
                            ...
                            <a href="{{ route('sync-logs.index', ['subdomain' => request()->route('subdomain')]) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded inline-block">
                                Reset
                            </a>
                        </div>
                    </form>
                                ...
                                @forelse ($logs as $log)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            <a href="{{ route('sync-logs.show', ['log' => $log, 'subdomain' => request()->route('subdomain')]) }}" class="text-blue-600 hover:text-blue-900">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('sync-logs.show', ['log' => $log, 'subdomain' => request()->route('subdomain')]) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                            @if ($log->status === 'failed' && $log->order)
                                                <form method="POST" action="{{ route('sync-logs.retry', ['order_id' => $log->order_id, 'subdomain' => request()->route('subdomain')]) }}" class="inline">
                                                    @csrf
