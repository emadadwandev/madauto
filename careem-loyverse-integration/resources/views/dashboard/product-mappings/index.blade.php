<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('product-mappings.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                Add New Mapping
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="p-6">
                    <div class="flex flex-wrap gap-2">
                        <form method="POST" action="{{ route('product-mappings.auto-map') }}" class="inline">
                            @csrf
                        ...
                        <a href="{{ route('product-mappings.export') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded inline-block">
                            Export to CSV
                        </a>
                        ...
                        <form method="POST" action="{{ route('product-mappings.clear-cache') }}" class="inline">
                            @csrf
                    ...
                    <div id="importForm" class="hidden mt-4">
                        <form method="POST" action="{{ route('product-mappings.import') }}" enctype="multipart/form-data">
                            @csrf
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="p-6">
                    <form method="GET" action="{{ route('product-mappings.index') }}" class="flex flex-wrap gap-4">
                        ...
                        <a href="{{ route('product-mappings.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded inline-block">
                            Reset
                        </a>
                    </form>
                </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($mappings as $mapping)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <form method="POST" action="{{ route('product-mappings.toggle', $mapping) }}" class="inline">
                                                @csrf
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('product-mappings.edit', $mapping) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                                            <form method="POST" action="{{ route('product-mappings.destroy', $mapping) }}" class="inline" onsubmit="return confirm('Are you sure?')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                            No product mappings found. <a href="{{ route('product-mappings.create') }}" class="text-blue-600">Create one</a> or try auto-mapping.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
