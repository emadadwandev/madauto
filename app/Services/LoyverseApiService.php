...
class LoyverseApiService
{
    ...
    public function __construct(ApiCredentialRepository $apiCredentialRepository)
    {
        ...
        // Get credentials from tenant-specific database first
        try {
            $this->token = $apiCredentialRepository->getCredential('loyverse', 'access_token');
        } catch (\Exception $e) {
            \Log::warning('Failed to load tenant Loyverse credentials', ['error' => $e->getMessage()]);
            $this->token = null;
        }

        // Only fall back to .env in local development environment
        if (empty($this->token) && app()->environment('local')) {
            $this->token = env('LOYVERSE_ACCESS_TOKEN');
            \Log::info('Using fallback Loyverse token from .env for local development');
        }
    ...
    protected function ensureCredentials(): void
    {
        if (! $this->hasCredentials()) {
            throw new \RuntimeException('Loyverse API credentials not configured for this tenant. Please add your Loyverse access token in the API Credentials section of your dashboard.');
        }
    }
